...
| 模型 | 词表 | 单步 | 吞吐 | 备注 |
|---|---|---|---|---|
| 512/10/8（47.9M） | 32k | 0.138s（compile）/ 0.229s（eager） | 29.7k / 17.9k tok/s | 纯训练稳态（不含 eval/存档） |
| 512/10/8（47.9M） | 32k | **0.17–0.19s** | **21–24k tok/s** | **端到端**（每 2k 步 eval+存档、含重启），排期用这行 |
...
> 前两行差 20~40%：稳态数字来自短测，排期必须用端到端——16.92 亿 tokens 的生产 run
> （v5，412k 步 × 4096 tokens/步）实测端到端 21~24k tok/s，按稳态 29.7k 排期会低估 4~6 小时。
