...
> 开训前务必跑一次 **完整体检**：`python scripts/data/audit_dataset.py`（有 BLOCKER 会返回 1）。
...
| **④ CoT 训练** | `sft/sft_cot_easy_em50_60k.jsonl`（easy）<br>`sft/sft_cot_hard_80k.jsonl`（hard） | 合成：`scripts/data/build_cot_sft.py`（算术题面，`--easy-max 50`）<br>+ 25% 混入 `BelleGroup` 通用指令 | 6 万条 + 8 万条 |
...
- ③ 的 14.5M 是**实测**（`python scripts/data/audit_sft_lengths.py`，对前 4 万条按 ctx512 真跑一遍
...
python scripts/data/parquet_to_jsonl.py --src dataset/chinese_cosmopedia \
...
python scripts/data/mix_corpora.py --source dataset/pretrain_t2t.jsonl:0.80 \
...
# 领域语料（以代码为例）
python scripts/data/parquet_to_jsonl.py --src dataset/IndustryCorpus2_computer_programming_code_high \
